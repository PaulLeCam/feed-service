...
async function run() {
  ...
  const [sharedServerFeed, singleServerFeed] = await Promise.all([alice.setFeed('alice-bob', sources), bob.setFeed('hello', [{
    user: bob.address
  }]), setContent(alice, 'Hello from Alice'), setContent(bob, 'Hello from Bob')]);

  if (sharedServerFeed.topic !== (0, _auth.hashFeeds)(sources)) {
